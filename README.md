...
```
KhmerSignLanguage/
├── dataset/                  ← Raw video clips (optional)
├── data/                     ← Processed landmark .npy files (gitignored — regenerate locally)
│   ├── hello/0/landmarks.npy
│   ├── X.npy / y.npy / labels.npy
│   └── X_train/X_val/X_test.npy, y_train/y_val/y_test.npy  (fixed split)
├── models/                   ← Saved trained model weights (*.pt, gitignored if >50MB — see below)
